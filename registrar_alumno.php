...
if (isset($_POST['enviar'])) {
    ...
    //tema nro matricula
    $consultaCantidadAlumnos = "SELECT rut FROM ALUMNO WHERE anio_alumno='$anio'";
    $resultadosCantidad = $dataBase->query($consultaCantidadAlumnos)->fetchAll(PDO::FETCH_OBJ);
    $cantidad = count($resultadosCantidad) + 1;
    //Insertando datos alumno
    $insertarAlumno = "INSERT INTO alumno (nro_matricula,a_paterno, a_materno, nombres, sexo ,rut, f_nacimiento, edad, domicilio, comuna, problema_salud, anio_alumno, fecha_egreso, Familiar_identificador_fam, Sige_identificador_sige, Escolar_identificador_esc, Retiro_identificador_retiro, Apoderado_identificador_apod, Curso_identificador_curso)"
            . "VALUES (:nro_mat,:pat,:mat,:nom,:sexo,:rut,:fnac,:edad,:dom,:com, :prob_sal, :anio, :fec_egre, :idfam, :idsige, :idesc, :idret, :idapod, :idCurso)";
    $resultadoInsertarAlumno = $dataBase->prepare($insertarAlumno);
    $resultadoInsertarAlumno->execute(array(":nro_mat" =>$cantidad,":pat" => $paterno, ":mat" => $materno, ":nom" => $nombre,
        ":sexo" => $sexo, ":rut" => $run, ":fnac" => $nacimiento, ":edad" => $edad, ":dom" => $domicilio, ":com" => $comuna,
        ":prob_sal" => $enfermedad, ":anio" => $anio, ":fec_egre" => $fecha_egreso, ":idfam" => $id, ":idsige" => $id, ":idesc" => $id, ":idret" => $id, ":idapod" => $id, ":idCurso" => $curso));
